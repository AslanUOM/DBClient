...
public class DBHandler {
	...
	private final String SELECT_LOCATION_TABLE = "SELECT "
			+ LOCATION_COLUMN_TIME + "," + LOCATION_COLUMN_PROVIDER + ","
			+ LOCATION_COLUMN_LATITUDE + "," + LOCATION_COLUMN_LONGITUDE + ","
			+ LOCATION_COLUMN_ACCURACY + " FROM " + LOCATION_TABLE_NAME + ";";
	private final String SELECT_WIFI_TABLE = "SELECT " + WIFI_COLUMN_TIME + ","
			+ WIFI_COLUMN_SSID + "," + WIFI_COLUMN_BSSID + ","
			+ WIFI_COLUMN_LEVEL + " FROM " + WIFI_TABLE_NAME + ";";
	...
	public void readDB() {
		...
		try {
			...
			c = DriverManager
					.getConnection("jdbc:sqlite:/home/gobinath/Workspace/university/DBClient/vishnu_location_db_dump_full.db");
			...
			while (rs1.next()) {
				...
				loc.setTimeStamp(Timestamp.valueOf(rs1
						.getString(LOCATION_COLUMN_TIME)));
			...
			// System.out.println(locationList.get(k).getRoundedTimeStamp() +
			// ", " + locationList.get(k).getTimeStamp());
			// }
			...
			while (rs2.next()) {
				WiFi wifi = new WiFi();
				wifi.setTimeStamp(Timestamp.valueOf(rs1
						.getString(WIFI_COLUMN_TIME)));
			...
			// System.out.println(wifiList.get(k).getRoundedTimeStamp() + ", " +
			// wifiList.get(k).getTimeStamp());
			// }
	...
	public void processData() {
		...
		SensorData lastLocation = null;
		for (; countLoc < locationList.size() - 1; countLoc++) {
			if (lastLocation == null
					|| lastLocation.getAccuracy() > locationList.get(countLoc)
							.getAccuracy()) {
				SensorData locData = new SensorData();
				locData.setType("location");
				locData.setSource(locationList.get(countLoc).getProvider());
				locData.setTime(locationList.get(countLoc).getTimeStamp()
						.getTime());
				locData.setAccuracy(locationList.get(countLoc).getAccuracy());
				locData.setData(new String[] {
						locationList.get(countLoc).getLatitude(),
						locationList.get(countLoc).getLongitude() });
				
				if(lastLocation != null) {
					response.getSensorDatas().remove(lastLocation);
				}
				response.addSensorData(locData);
				lastLocation = locData;
			}
			if (locationList.get(countLoc).equals(
					locationList.get(countLoc + 1))) {
				continue;
			}

			List<String> bssids = new ArrayList<>();
			for (; countWifi < wifiList.size() - 1; countWifi++) {
				if (locationList.get(countLoc).getRoundedTimeStamp().getTime() < wifiList
						.get(countWifi).getRoundedTimeStamp().getTime()) {
					break;
				} else if (locationList.get(countLoc).equals(
						wifiList.get(countWifi))) {
					bssids.add(wifiList.get(countWifi).getBSSID());
					if (wifiList.get(countWifi).equals(
							wifiList.get(countWifi + 1))) {
						continue;
					}
					SensorData wifiData = new SensorData();
					wifiData.setType("networks");
					wifiData.setSource("wifi");
					wifiData.setTime(wifiList.get(countWifi).getTimeStamp()
							.getTime());
			...
			if(response.getSensorDatas() != null && response.getSensorDatas().size() == 2) {
				sendData(response);
				print(response);
			}
			lastLocation = null;
